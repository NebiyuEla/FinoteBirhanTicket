(() => {
  ...
  document.addEventListener('touchmove', (event) => {
    if (!event.target.closest('.number-grid') && !event.target.closest('input') && !event.target.closest('.seller-note')) event.preventDefault();
  }, { passive: false });
  ...
  const prizeNames = {
    am: { 200: params.get('p200am') || 'ዋና ሽልማት', 100: params.get('p100am') || 'የእንጨት መጽሐፍ ቅዱስ', 50: params.get('p50am') || 'የእመቤታችን ምስል + ቁልፍ ማንጠልጠያ' },
    en: { 200: params.get('p200en') || 'Grand Prize', 100: params.get('p100en') || 'Wooden Bible', 50: params.get('p50en') || 'St. Mary icon + keychain' }
  };
  ...
  const T = {
    am: {
      brandKicker:'ፍኖተ ብርሃን ሰ/ት/ቤት', brandTitle:'ዕጣ ትኬት', chooseTicket:'ትኬት ይምረጡ', chooseTicketHint:'የሚያሸንፉትን ሽልማት ይመልከቱ፣ ትኬት ይምረጡ።',
      discount:'ቅናሽ', bundleTitle:'ሶስቱም በአንድ', bundlePrize:'ሶስቱንም ሽልማቶች የማሸነፍ ዕድል', birr:'ብር', tapHint:'አንዱን ትኬት በመጫን ቁጥር ይምረጡ',
      chooseNumber:'ቁጥር ይምረጡ', back:'ተመለስ', available:'ያለ', selected:'የተመረጠ', taken:'የተወሰደ', reserveContinue:'ያስይዙ እና ይቀጥሉ',
      newSale:'አዲስ ትኬት ሽያጭ', buyerInfo:'የገዢውን መረጃ ያስገቡ', buyerFullName:'የገዢው ሙሉ ስም', phone:'ስልክ ቁጥር', paymentDestination:'ክፍያ የት ይገባ?',
      finoteBirhan:'ፍኖተ ብርሃን', sellerAccount:'የእኔ አካውንት', chooseTicketArrow:'ትኬት ይምረጡ →', edit:'ቀይር', expiredTitle:'ሊንኩ አልፎታል', expiredText:'ወደ ቦቱ ይመለሱ እና “ትኬት ይግዙ” በመጫን እንደገና ይክፈቱ።',
      sellerPayNote:'ገዢው ወደ እርስዎ አካውንት ይከፍላል። ገንዘቡን ካዩ በኋላ “ተሽጧል” ይጫኑ።', finotePayNote:'ገዢው ወደ ፍኖተ ብርሃን አካውንት ይከፍላል። ክፍያውን ካረጋገጡ በኋላ “ተሽጧል” ይጫኑ።',
      chooseOne:'ከ001–200 አንድ ያልተወሰደ ቁጥር ይምረጡ።', chooseBundle:'ከእያንዳንዱ ዕጣ አንድ ቁጥር ይምረጡ።', chooseNumberShort:'ቁጥር ይምረጡ', sending:'በመላክ ላይ…', sellerMode:'የትኬት ሻጭ', sellerUnconfigured:'የእኔ አካውንት · አልተዘጋጀም', fullNamePlaceholder:'ሙሉ ስም'
    },
    en: {
      brandKicker:'FinoteBirhan Sunday School', brandTitle:'Digital Ticket', chooseTicket:'Choose a ticket', chooseTicketHint:'See the prize clearly, then choose your ticket.',
      discount:'SAVE', bundleTitle:'All 3 in one', bundlePrize:'A chance to win from all three prize draws', birr:'ETB', tapHint:'Tap a ticket to choose your number',
      chooseNumber:'Choose a number', back:'Back', available:'Available', selected:'Selected', taken:'Taken', reserveContinue:'Reserve & continue',
      newSale:'New ticket sale', buyerInfo:'Enter the buyer information', buyerFullName:'Buyer full name', phone:'Phone number', paymentDestination:'Where will the buyer pay?',
      finoteBirhan:'FinoteBirhan', sellerAccount:'My account', chooseTicketArrow:'Choose ticket →', edit:'Edit', expiredTitle:'This link expired', expiredText:'Return to the bot and tap “Buy Ticket” to open a fresh page.',
      sellerPayNote:'The buyer pays your account. After you see the money, tap SOLD.', finotePayNote:'The buyer pays the FinoteBirhan account. After payment is confirmed, tap SOLD.',
      chooseOne:'Choose one available number from 001–200.', chooseBundle:'Choose one number from each draw.', chooseNumberShort:'Choose a number', sending:'Sending…', sellerMode:'Ticket Seller', sellerUnconfigured:'My account · Not configured', fullNamePlaceholder:'Full name'
    }
  };
  ...
  document.querySelectorAll('[data-lang-control],#langToggle').forEach((btn) => btn.addEventListener('click', toggleLanguage));
  ...
  [payFinote, paySeller].forEach((button) => button.addEventListener('click', () => {
    ...
    updatePaymentNote(); updateSellerBuyerMeta();
  }));
  ...
  confirmBtn.addEventListener('click', () => {
    ...
    confirmBtn.disabled = true; confirmBtn.textContent = T[lang].sending;
    try { tg.sendData(JSON.stringify(payload)); }
    catch { confirmBtn.disabled = false; confirmBtn.textContent = T[lang].reserveContinue; show('error'); }
  });
  ...
  async function loadTicketImages() {
    ...
    await Promise.all(definitions.map(async ({id,parts}) => {
      ...
      try {
        const chunks = await Promise.all(parts.map(async (path) => { const response = await fetch(path,{cache:'force-cache'}); if (!response.ok) throw new Error(path); return response.text(); }));
  ...
  function applyLanguage() {
    ...
    document.querySelectorAll('[data-lang-control],#langToggle').forEach((btn) => { btn.textContent = lang === 'am' ? 'EN' : 'አማ'; });
  ...
  function updatePaymentNote() {
    document.getElementById('sellerPaymentNote').textContent = state.paymentTarget === 'seller' ? T[lang].sellerPayNote : T[lang].finotePayNote;
  }
  ...
  function render() {
    document.querySelectorAll('.pool-tab').forEach((button) => {
      const pool = Number(button.dataset.pool); button.classList.toggle('active', pool === state.activePool); button.classList.toggle('has-value', Boolean(state.numbers[pool]));
    ...
    for (let number = 1; number <= 200; number += 1) {
      const button = document.createElement('button'); button.type='button'; button.className='num'; button.dataset.number=String(number); button.textContent=formatNumber(number);
      if (unavailable[state.activePool].has(number)) { button.disabled=true; button.classList.add('taken'); }
      else if (state.numbers[state.activePool] === number) button.classList.add('selected');
      fragment.appendChild(button);
    }
    numberGrid.replaceChildren(fragment);
    if (state.ticket === 'bundle') summary.innerHTML = [200,100,50].map((pool) => `<strong>${pool} ${T[lang].birr}:</strong> ${state.numbers[pool] ? `#${formatNumber(state.numbers[pool])}` : '—'}`).join(' · ');
    else if (state.ticket) { const pool = Number(state.ticket); summary.innerHTML = `<strong>${pool} ${T[lang].birr}:</strong> ${state.numbers[pool] ? `#${formatNumber(state.numbers[pool])}` : T[lang].chooseNumberShort}`; }
    confirmBtn.disabled = !complete(); if (!confirmBtn.disabled || confirmBtn.textContent !== T[lang].sending) confirmBtn.textContent = T[lang].reserveContinue;
  }
  ...
  function decodeBitset(encoded) {
    const output = new Set(); if (!encoded) return output;
    try { const normalized=encoded.replace(/-/g,'+').replace(/_/g,'/'); const padding='='.repeat((4-normalized.length%4)%4); const raw=atob(normalized+padding); for (let index=0;index<Math.min(200,raw.length*8);index+=1){ const byte=raw.charCodeAt(Math.floor(index/8)); if(byte&(1<<(index%8))) output.add(index+1); } } catch {}
    return output;
  }
  render();
})();
