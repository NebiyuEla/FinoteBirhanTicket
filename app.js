(() => {
  ...
  if (tg) {
    tg.ready();
    tg.expand();
    try { tg.setHeaderColor('#fffdf8'); tg.setBackgroundColor('#f7f1e7'); } catch {}
  }
  ...
  sellerLine.textContent = sellerName
    ? `የሽያጭ ወኪል: ${sellerName}`
    : 'ትኬት እና ቁጥር ይምረጡ።';

  if (!session || !tg?.sendData) {
    show('error');
  }
  ...
  confirmBtn.addEventListener('click', () => {
    ...
    const pools = state.ticket === 'bundle'
      ? [200, 100, 50]
      : [Number(state.ticket)];

    for (const pool of pools) {
      payload.numbers[String(pool)] = state.numbers[pool];
    }
  ...
  function render() {
    ...
    for (let number = 1; number <= 200; number += 1) {
      ...
      if (unavailable[state.activePool].has(number)) {
        ...
        button.setAttribute('aria-label', `Number ${number} taken`);
